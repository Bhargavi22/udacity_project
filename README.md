...
The project manager along with business analysts identified below open data sources that suits the requirement of this project-

- Covid Data - John Hopkins University &amp; Medicine – Coronavirus resource centre
- Mobility Data – Google&#39;s Community Mobility Reports

They would like a Data Engineer to create a data pipeline to ingest this data into our system, create a data model that fits the purpose and make the data available for Data Analysts and BI developers to build the visualizations that can eventually be embedded on Mediafy's webpage.
...
- Clean the data.
- Load cleaned data into Redshift to make it available for the BI developers to gather insights from the data.

### **Choice of Tools**

- Airflow for data pipelining &amp; scheduling.</br>
   Why airflow?
    - Covid data &amp; Google mobility data are time-based &amp; are updated daily. There are multiple steps involved in getting the final data. With airflow, the process can be run daily, and it also provides a user-friendly view of the data pipeline, making the monitoring easier.
...
- Redshift for data storage.</br>
   Why Redshift?
    ...
    - Storing data in Redshift would provide ease in querying the data from visualization tools such as Tableau and Power BI. which can connect directly to the Redshift warehouse.
...
| Granularity of data  | For countries other than US – By state and country   </br>For US only – by city/county and, occasionally by state  |
| | |
...
| Granularity of data  | For countries other than US – Country level, for a few countries at state levewl </br>For US only – county level and, at state level for a few states  |
| | |
...
| Granularity of data  | For countries other than US – Country Level, for a few countries at state and city level. </br> For US only – At city/county level, occasionally at state level  |
| | |
</br>
...
Based on the requirements &amp; assessment of covid data &amp; google mobility data had following things in common-
...
2. Keeping the US data separate helps improve query performance.
...
4. The reporting &amp; accuracy of US data is higher compared to the rest of the world.
...
2. **load\_covid\_and\_mobility\_data\_daily.py** 
    ...
    - Flow of the DAG at a high level [Click here for details](#flow-diagram-of-daily-dag-with-explanation)
      1. Create Tables in Redshift if they do not exist
...
![load_covid_and_mobility_data_daily](./dags/load_covid_and_mobility_data_daily.png)

  **Explanation of each task -**

| **Seq No** | **Task Id** | **Task operator** | **Functionality** |
| --- | --- | --- | --- |
| **1** | create\_redshift\_table   | create\_redshift\_tables.py   | Create staging &amp; final tables if they do not exist. The table definitions are stored in helpers/ create\_tables.py   |
...
- **If the data was increased by 100x.:**  
  - We could have different tables based on continents
  - Increase the size of redshift cluster
  - Use Spark for processing and redshift for storage
- **If the pipelines were run on a daily basis by 7am.:**
  - The current pipeline is well equipped for this scenario as it is pulling filed daily
